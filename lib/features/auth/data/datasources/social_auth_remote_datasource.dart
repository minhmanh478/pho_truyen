...
abstract class SocialAuthRemoteDataSource {
  Future<AuthModel> loginSocial(SocialLoginRequest request);
}

class SocialAuthRemoteDataSourceImpl implements SocialAuthRemoteDataSource {
  final DioClient dioClient;

  SocialAuthRemoteDataSourceImpl({required this.dioClient});

  @override
  Future<AuthModel> loginSocial(SocialLoginRequest request) async {
    try {
      ...
      if (authData.token.isNotEmpty) {
        ...
        print('Social Login: Access Token saved');
      }
      ...
      throw DioException(
        requestOptions: RequestOptions(path: '/api/auth/login-social'),
