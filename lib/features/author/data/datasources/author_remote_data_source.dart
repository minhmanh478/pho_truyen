// ignore_for_file: avoid_print

import 'package:dio/dio.dart';
import 'package:get/get.dart';
import 'package:pho_truyen/core/network/api_client.dart';
import 'package:pho_truyen/core/network/base_response.dart';
import 'package:pho_truyen/features/author/data/models/author_model.dart';

abstract class AuthorRemoteDataSource {
  Future<AuthorDetailResponse> getAuthorDetail(int id);
  Future<List<dynamic>> getStoriesByAuthor(int userId, int offset, int limit);
}

class AuthorRemoteDataSourceImpl implements AuthorRemoteDataSource {
  final DioClient dioClient = Get.find();

  @override
  Future<AuthorDetailResponse> getAuthorDetail(int id) async {
    try {
      final response = await dioClient.dio.post(
        '/api/author/detail',
        data: {'id': id},
      );
      return AuthorDetailResponse.fromJson(response.data);
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
      throw DioException(
        requestOptions: RequestOptions(path: '/api/author/detail'),
        error: "Lỗi không xác định: $e",
      );
    }
  }

  @override
  Future<List<dynamic>> getStoriesByAuthor(
    int userId,
    int offset,
    int limit,
  ) async {
    try {
      final response = await dioClient.dio.post(
        '/api/story/story-in-user',
        data: {
          'user_id': userId,
          'search': '',
          'offset': offset,
          'limit': limit,
        },
      );
      final data = response.data;
      print('DEBUG: Author Stories Response Data: $data');

      if (data is Map<String, dynamic>) {
        // Check for nested structure: data -> data -> items
        if (data['data'] is Map<String, dynamic> &&
            data['data']['items'] is List) {
          print('DEBUG: Found items list in data["data"]["items"]');
          return data['data']['items'];
        }
        // Check if data['data'] is directly a list (just in case)
        if (data['data'] is List) {
          print('DEBUG: Found data list in data["data"]');
          return data['data'];
        }
      } else if (data is List) {
        print('DEBUG: Data is list');
        return data;
      }

      print('DEBUG: Data format not recognized or empty');
      return [];
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
      throw DioException(
        requestOptions: RequestOptions(path: '/api/story/story-in-user'),
        error: "Lỗi không xác định: $e",
      );
    }
  }

  DioException _handleDioError(DioException e) {
    String errorMessage = 'Có lỗi xảy ra, vui lòng thử lại.';

    if (e.response != null && e.response?.data != null) {
      final data = e.response!.data;
      if (data is Map<String, dynamic>) {
        try {
          final baseResponse = BaseResponse<dynamic>.fromJson(
            data,
            (json) => json,
          );
          errorMessage = baseResponse.message;
        } catch (_) {
          errorMessage = data['message'] ?? data['error'] ?? errorMessage;
        }
      } else if (data is String) {
        if (data.contains('<!DOCTYPE html>') || data.contains('<html')) {
          errorMessage = 'Lỗi máy chủ (HTML Response). Vui lòng liên hệ admin.';
        } else {
          errorMessage = data;
        }
      }
    } else {
      switch (e.type) {
        case DioExceptionType.connectionTimeout:
        case DioExceptionType.sendTimeout:
        case DioExceptionType.receiveTimeout:
          errorMessage = "Hết thời gian kết nối. Vui lòng kiểm tra mạng.";
          break;
        case DioExceptionType.connectionError:
          errorMessage = "Không có kết nối Internet.";
          break;
        case DioExceptionType.cancel:
          errorMessage = "Yêu cầu đã bị hủy.";
          break;
        case DioExceptionType.unknown:
          if (e.error.toString().contains("SocketException")) {
            errorMessage = "Không thể kết nối đến máy chủ. Kiểm tra mạng/VPN.";
          } else if (e.error.toString().contains("HandshakeException")) {
            errorMessage = "Lỗi chứng chỉ bảo mật (SSL).";
          } else {
            errorMessage = "Lỗi hệ thống: ${e.error}";
          }
          break;
        default:
          errorMessage = e.message ?? "Lỗi không xác định.";
          break;
      }
    }

    return DioException(
      requestOptions: e.requestOptions,
      response: e.response,
      error: errorMessage,
      type: e.type,
    );
  }
}
